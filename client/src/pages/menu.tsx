      ...
      <Button
        ...
        className={`inline-flex items-center ${
          !isAcceptingOrders
            ...
            : "bg-[#fff9dc] hover:bg-[#fffcea] text-black hover:shadow-lg"
        } font-bold shadow-md transition-all border ${!isAcceptingOrders ? "border-gray-300" : "border-[#e80113]/30"}`}
    ...
    <Button
      ...
      className={`inline-flex items-center ${
        !isAcceptingOrders
          ...
          : "bg-[#e80113] hover:bg-[#d10010] text-white hover:shadow-lg"
...
export default function Menu() {
  ...
  const { isAcceptingOrders, refetch: refetchStoreSettings } =
    useStoreSettings(); // 店舗の注文受付状態
  ...
  const handleQuickBuy = (product: Product) => {
    ...
    if (!isAuthenticated) {
      toast({
        title: "ログインしてください",
        description:
          "商品を注文するには、ログインが必要です。画面右上のログインボタンからお進みください。",
        variant: "destructive",
      });
      return;
    }
  ...
  const openProductDialog = (product: Product) => {
    ...
    if (!isAuthenticated) {
      toast({
        title: "ログインしてください",
        description:
          "商品をカスタマイズするには、ログインまたは会員登録が必要です。画面右上のログインボタンからお進みください。",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-10 bg-white py-8 rounded-lg shadow-lg border-2 border-[#e80113] relative overflow-hidden">
        ...
        <h1 className="text-4xl font-bold text-[#e80113] mb-3 font-japanese">
          味店焼マン
        </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {products
                ...
                .map((product) => (
                  <Card
                    key={product.id}
                    className="overflow-hidden border-2 border-[#e80113] shadow-md hover:shadow-lg transition-all duration-300 hover:scale-[1.02]"
                  >
                    ...
                    <CardContent className="p-5 bg-gradient-to-b from-white to-[#fffcf0]">
                      <h3 className="text-xl font-bold text-[#e80113]">
                        {product.name}
                      </h3>
                      ...
                      {product.name === "から丼" && (
                        <div className="mt-2 text-xs text-gray-600">
                          ご飯大 480円　おかず大 540円　大大 600円　ガールズ
                          380円
                        </div>
                      )}
                      {(product.name === "キムカラ丼" ||
                        ...
                        product.name === "から玉子丼") && (
                        <div className="mt-2 text-xs text-gray-600">
                          ご飯大 590円　おかず大 700円　大大 760円　ガールズ
                          490円
                        </div>
                      )}
                      {product.name === "月見カルビ丼" && (
                        <div className="mt-2 text-xs text-gray-600">
                          ご飯大 660円　おかず大 760円　大大 830円　ガールズ
                          550円
                        </div>
                      )}
                      {product.name === "デラ丼" && (
                        <div className="mt-2 text-xs text-gray-600">
                          ご飯大 770円　おかず大 880円　大大 940円　ガールズ
                          550円
                        </div>
                      )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {products
                ...
                .map((product) => (
                  <div
                    key={product.id}
                    className="flex items-center justify-between bg-gradient-to-r from-white to-[#fffaee] p-4 rounded-lg border-2 border-[#e80113]/40 shadow-sm hover:shadow-md transition-all duration-300 hover:border-[#e80113]"
                  >
                    <div>
                      <h3 className="font-bold text-[#e80113] text-lg">
                        {product.name}
                      </h3>
                    ...
                    <div className="flex items-center space-x-3">
                      <div className="font-bold text-lg text-white bg-[#e80113] px-2 py-1 rounded-md shadow-sm">
                        ¥{product.price}
                      </div>
                      <OrderButton
                        isAcceptingOrders={isAcceptingOrders}
